...
SPAM_PATTERNS = [r"judi\b", r"slot\b", r"togel\b", r"casino\b|kasino\b", r"\bmaxwin\b", r"scatter\b", r"\brtp\b", r"pola\s+gacor", r"bonus\s+new\s+member", r"\bparlay\b", r"\b4d\b|\b3d\b|\b2d\b", r"\bwd\b", r"deposit\b.*(pulsa|e[-\s]?wallet)", r"\bbandar\b|\btaruhan\b|\bbet\b", r"pragmatic|habanero|pg\s*soft", r"(wa|whatsapp)[:\s]*\+?\d{8,}", r"http[s]?://\S+", r"link\s+alternatif"]
SPAM_REGEX = re.compile("|".join(SPAM_PATTERNS), re.IGNORECASE)
def normalize_text(s): return unicodedata.normalize("NFKC", s or "")
def is_spam(text): return bool(SPAM_REGEX.search(normalize_text(text)))
...
def delete_comment(yt, cid: str): yt.comments().delete(id=cid).execute()

@app.get("/")
def index():
    acc_id = session.get("acc_id")
    if not acc_id: return render_template("index.html")
...
def oauth_callback():
    ...
    creds = flow.credentials; refresh_token = creds.refresh_token
    if not refresh_token: return "No refresh_token received; remove previous consent and login again.", 400
    ...
    with SessionLocal() as db:
        ...
        if row:
            row.channel_title = channel_title; row.refresh_token = refresh_token; db.commit(); acc = row
        else:
            acc = Account(email="user", channel_id=channel_id, channel_title=channel_title, refresh_token=refresh_token); db.add(acc); db.commit()
...
def scan_my_channel():
    acc_id = session.get("acc_id")
    if not acc_id: return jsonify({"error": "Not logged in"}), 401
    with SessionLocal() as db:
        acc = db.get(Account, acc_id); yt = yt_from_refresh(acc.refresh_token)
        flagged=[]; deleted=0; scanned=0
        for thread in list_channel_threads(yt, acc.channel_id):
            for cid, text, author, is_reply in iter_comments(thread):
                ...
                if is_spam(text):
                    ...
                    try: delete_comment(yt, cid); deleted += 1
                    except Exception as e: flagged[-1]["delete_error"] = str(e)
        summary={"channel_id": acc.channel_id, "channel_title": acc.channel_title, "scanned": scanned, "flagged": len(flagged), "deleted": deleted, "examples": flagged[:10], "ts": datetime.utcnow().isoformat()+"Z"}
        acc.last_scan_at = datetime.utcnow(); acc.last_scan_summary = json.dumps(summary); db.commit()
    return jsonify(summary)

@app.post("/cron/scan_all")
def cron_scan_all():
    if request.args.get("key") != CRON_KEY: return jsonify({"error":"forbidden"}), 403
    results = []
    with SessionLocal() as db:
        for acc in db.query(Account).all():
            try:
                yt = yt_from_refresh(acc.refresh_token); flagged=[]; deleted=0; scanned=0
                for thread in list_channel_threads(yt, acc.channel_id):
                    for cid, text, author, is_reply in iter_comments(thread):
                        scanned += 1
                        if is_spam(text):
...
if __name__ == "__main__":
    if os.getenv("ENV", "dev") == "dev": os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = "1"
